...
$query  = "SELECT * FROM cdd_comunicados61 WHERE comunicado = ".$_GET['id'];
...
<div align="center">	
	...
	<div id="contenido">
		<div id="contenido_left">
			...
			<p class="date" style="font-size: 1.0em; text-align: right;">Toluca de Lerdo, México, <? echo substr($row['fecha'],8,2); ?> de <? echo $mes_txt ?> de <? echo substr($row['fecha'],0,4); ?></p>
...
<table>
	...
	<tbody>
		...
		<tr>
		  ...
		  <td>Mónica Miriam Granillo Velazco </td>
			...
			<div style="background: rgba(0,0,0,0.6); padding: 15px;">
				<h3 style="color: white;"><i class="fas fa-camera-retro"></i> Fotografías</h3>
				<?
				...
				?>
				<a href="../img/fotos/comunicados/<? echo $pic['foto']; ?>" rel="shadowbox[album]" title="<? echo utf8_encode($row['titulo']); ?>"><div style="display: block; margin: 5px; float: left; padding: 0px; box-shadow: rgba(0,0,0,0.5) 2px 2px 2px; width: 180px; height:120px; background: url(../img/fotos/comunicados/<? echo $pic['foto']; ?>) 50% 50%; background-size: cover;"></div></a>
				<? } ?>	
		...
		<!--<div style="width: 90%; max-width: 330px; float: left; margin:10px; display: block; text-align: justify;">-->
			...
			<?
			$Coneccion = new TConeccion();
			$query  = "SELECT * FROM cdd_comunicados61 WHERE comunicado != ".$row['comunicado']." ORDER BY fecha DESC, comunicado DESC LIMIT 3";
			$Coneccion->Gestion($query);
